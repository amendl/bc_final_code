...
if __name__=="__main__":
    ...
    for i in geometry.LineScanOHBond01.generator():
        ...
        CC1,HF1,c1,d1,e1,f1,energy_from_monomers1,multipoles = process_file(f"../oh_bond_2_dipole/{format(i+1,'02')}_AVTZ/molpro.out",i)
        CC2,HF2,c2,d2,e2,f2,energy_from_monomers2,_ = process_file(f"../oh_bond_2_dipole/{format(i+1,'02')}_AVQZ/molpro.out",i)
    ...
    # _from = 0
    # with lib.PlottingContextManager("orders_comparison_from_d_matrix_t",[".png",".pdf"]) as (fig,ax):
    ...
    #     plt.xlabel("Angstrom")
    #     lib.plot_serie(x[_from:],E[_from:],label="CCSD(T)",color="yellow")

    _from = 4
    with lib.PlottingContextManager("orders_comparison_from_d_matrix_AVTZ",[".png",".pdf"]) as (fig,ax):
        lib.plot_serie(x[_from:],E_HF[_from:],label="HF",color="black")
        for i,values in enumerate(M):
            print(i,values)
            lib.plot_serie(x[_from:],np.array(values[_from:]),label=f"multipoles up to order {i}",color=None)
