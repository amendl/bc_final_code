...
if __name__=="__main__":
    ...
    # load mbe data
    cell=mbe.read_cell('/store/kchfo2/klimj2am/molec_crystals/CO2/input/cell')
    coord, elems=mbe.read_monomers_uni('/store/kchfo2/klimj2am/molec_crystals/CO2/input')
    with open('/home/users/mendla/work/3/data/CO2_new_energies_sym_shell6_test20tig_AVDZ_AVQZfit.dat') as f:
        calc_all=f.readlines()

    # load gdma data
    DMA_entries = [lib.DMA.get_from_gdam_file('/home/users/mendla/work/2/CO2_model_data/HF_gdma_V5Z/' + str(file_index)+'/dma4.punch') for file_index in range(4)]      
    ...
    data = np.zeros((len(calc),6))

    data_for_histogram = np.zeros(int(len(calc)/4))
    ...
    for idx,calc_line in enumerate(calc):
        ...
        try: 
            ...
            data[idx,1]=dma_energy
            data[idx,2]=float(calc_line.split()[-4])
            data[idx,3]=float(calc_line.split()[-3])
            data[idx,4]=float(calc_line.split()[-2])
            data[idx,5]=float(calc_line.split()[-1])
        except:
            pass
    ...
    print("Writing")
    with open('test_CO2.npy', 'wb') as f:
        np.save(f,data)
