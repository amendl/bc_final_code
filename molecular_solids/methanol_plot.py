...
if __name__=="__main__":
    ...
    data = np.load("../scripts/test.npy")[0:2667,:]
    # print(data)
    ...
    _from = 20
    _to = -1 
    # print(np.where(data[:,0]>60)[0][0])
    fig, ax = plt.subplots(figsize=(2.79077651389*2,3.8),constrained_layout=True)
    ...
    plt.legend()
    plt.savefig("methanol_sum_final.png",dpi=300)
    ...
    plt.xlabel("Cutoff distance / $a_0$")
    plt.legend()
    plt.savefig("methanol_sum_differences_final.png",dpi=300)
    ...
    plt.xlabel("Cutoff distance / $a_0$")
    plt.legend()
    plt.savefig("methanol_differences_final.png",dpi=300)
